...
8. Sovereign Economic Layer

iZCore v0.1.7+ introduces the foundations for a decentralized token economy:

Node Wallet: Every authenticated iZCore node is automatically provisioned with a unique cryptographic wallet address. This address is used to receive rewards and participate in the network's financial matrix.

Reward Distribution: Tokens are distributed to node owners based on "Proof of Contribution" (PoC). Rewards are calculated based on the computational resources provided (LLM Training), the value generated by active skills (Trading Bot profits), and the reliability of distributed storage nodes.
